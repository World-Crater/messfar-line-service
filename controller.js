...
async function textController(req, res) {
  try {
    switch (req.body.events[0].message.text) {
      case "許願":
        const [randomFaceResult, randomFaceError] =
          await recognitionService.randomFace();
          ...
          [
            {
              ...
              contents: {
                type: "carousel",
                contents: utils.arrayApplyToTemplage(
                  randomFaceResult,
            ...
            {
              type: "text",
              text: randomFaceResult.map((item) => item.name)[0],
            },
          ],
...
async function sendSearchFaceImage(req) {
  ...
  const savePath = "./temp";
  const [getImageResult, getImageError] = await lineService.getImage(
  ...
  const [searchFaceResult, searchFaceError] =
    await recognitionService.searchFace(`${savePath}/${imageName}`);
  if (searchFaceError) {
    // TODO: 回傳辨識失敗的訊息
  }
  const [, error] = await lineService.reply(
    [
      ...
      {
        type: "text",
        text: JSON.stringify(searchFaceResult.map((item) => item.name)),
      },
    ],
    req.body.events[0].replyToken
  );
